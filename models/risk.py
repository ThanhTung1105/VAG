...
risk_pics = db.Table('risk_pics',
    db.Column('risk_id', db.BigInteger, db.ForeignKey('risks.id'), primary_key=True),
...
class Risk(db.Model):
    ...
    pics = db.relationship('Member', secondary='risk_pics', lazy='selectin')
    ...
    __table_args__ = (db.Index('ix_risk_source', 'source_table', 'source_id'),)

    def to_dict(self):
        return {
            'id': self.id, 'type': 'risk',
